...
- `scripts/validation_numbers.py`, which regenerates the sample-count and start-index tables in `docs/VALIDATION_REPORT.md`, so those figures can be checked against the engine instead of taken on trust. The start-index tables used to be typed in by hand and did not reproduce: the serpentine's published 6.31% is 3.48% over the start indices its test tries.
...
- Python 3.14 is supported, and `requires-python` is now `>=3.11,<3.15`. CI runs the Python suite on 3.14 alongside 3.11–3.13, and the upper bound keeps the published wheel from admitting a Python version the suite has never run on; a test fails when the bound and the CI matrix name different versions.
...
- Canvas input lands under the pointer. The surface and the view box rarely share an aspect ratio, and the browser draws the view box at one uniform scale with bands on the longer side, but the pointer mapping stretched each axis across the whole surface. A point dragged near the top of Aurora Circuit on a 1280x720 desktop came to rest 14 px below the cursor on Chromium, Firefox and WebKit alike; panning lagged the hand along that axis, and a calibration span along it measured short, so the photo was scaled up by as much as that axis was stretched. None of the end-to-end specs dragged a point, so none could see it.
